...
if(isset($data['models'])){
    foreach ($data['models'] as $model){
        $tr = new \Framework\view\components\Table\Row();
        $th = new \Framework\view\components\Table\Th('px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap',$model->short_name);
        $tr->addLeave($th);

        $td = new \Framework\view\components\Table\Column('px-6 py-4',$model->name);
        $tr->addLeave($td);
